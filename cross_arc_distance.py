...
def get_bearing_vectorised(latA, longA, latB, longB):
    ...
    y = np.cos(latA) * np.sin(latB) - np.sin(latA) * \
        np.cos(latB) * np.cos(dlong)
...
def haversine_vectorised(latA, longA, latB, longB):
    ...
    a = np.sin((latB - latA) / 2) ** 2 + np.cos(latA) * \
        np.cos(latB) * np.sin((longB - longA) / 2) ** 2
...
def cross_arc_distance_vectorised(latA, longA, latB, longB, latC, longC, max_consideration_distance=1000):
    ...
    # TODO: Write a test for this!
    relative_bearing[relative_bearing > np.pi] = 2 * \
        np.pi - relative_bearing[relative_bearing > np.pi]
    ...
    dxt = cross_track_distance_vectorised(
        latA, longA, latB, longB, latC, longC)
    ...
    haversineAD = np.arccos(
        np.cos(haversineAC / EARTH_RADIUS) / np.cos(dxt / EARTH_RADIUS)) * EARTH_RADIUS
    ...
    # We return the haversineAB so we can use it for "how lit is a lit path" estimate,
    # and it's already been calculated for free here
    return result, haversineAB
